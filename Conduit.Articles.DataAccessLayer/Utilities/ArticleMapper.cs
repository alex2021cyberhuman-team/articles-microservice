...
{
    public static InternalArticleModel MapArticleToInternalArticleModel(this ArticleDbModel model,
        bool following = false,
        bool favorited = false) =>
        new(model.Id, model.Slug, model.Title, model.Description, model.Body,
            model.Tags.Select(x => x.Name).ToHashSet(), model.CreatedAt,
            model.UpdatedAt,
            favorited, model.FavoritesCount,
            new(model.AuthorId, model.Author.Username, model.Author.Bio, model.Author.Image,
                following));
    ...
    public static SingleArticle MapArticleToSingleArticle(
        ...
        bool following = false,
        bool favorited = false) =>
        new(new(model.Slug, model.Title, model.Description, model.Body,
            model.Tags.Select(x => x.Name).ToHashSet(), model.CreatedAt,
            model.UpdatedAt,
            favorited, model.FavoritesCount,
